...
for per in percents:
    ...
    for alpha in alphas:
        with open("../result/cifar10sub/alpha_{}_percent_{}/log.log".format(alpha, per), "r") as f:
            ...
            s = s[-10:-1]
            acc.append( np.array(s).mean() )

    accs.append(acc)
    plt.plot( alphas, acc, label="ratio={}%".format(per) )
    ...
    with open("../result/cifar10sub/alpha_0_percent_{}/log.log".format(per), "r") as f:
        ...
        for line in lines:
            if 'VAL FINAL' in line:
                s.append( float(line.split(" ")[-1]) )
        s = s[-10:-1]
        acc = [np.array(s).mean()] * len(alphas)
    plt.plot( alphas, acc, label='ratio={}%,baseline'.format(per) )
